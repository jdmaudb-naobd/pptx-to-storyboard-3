"""
Run the simplified analyzer to generate learned_patterns.json
"""
...
# Save the simplified analyzer to src
simple_analyzer_code = open('src/simple_analyzer.py', 'w', encoding='utf-8')
simple_analyzer_code.write('''"""
...
class SimplifiedAnalyzer:
    ...
    def _generate_patterns(self, all_patterns: Dict) -> Dict:
        ...
        return patterns
''')
simple_analyzer_code.close()

# Now import and run
from src.simple_analyzer import SimplifiedAnalyzer

def main():
    """Run simplified analysis or use defaults"""
    
    try:
        print("🚀 Attempting to analyze your examples...")
        analyzer = SimplifiedAnalyzer("examples")
        patterns = analyzer.analyze_all_examples()
        
    except Exception as e:
        print(f"\n⚠️  Analysis encountered issues: {str(e)}")
        print("\n📝 Creating default patterns based on eLearning best practices...")
        
        # Create sensible default patterns
        patterns = {
            'transformation_rules': {
                'omit_slide_types': ['disclosure', 'title', 'references'],
                'combine_slide_types': ['content', 'patient_case', 'clinical_data'],
                'avg_slides_per_segment': 2.5,
                'slide_type_priority': {
                    'objectives': 'always_include',
                    'patient_case': 'combine_with_content',
                    'clinical_data': 'combine_with_treatment',
                    'treatment': 'important',
                    'conclusion': 'always_include',
                    'questions': 'transform_to_assessment'
                }
            },
            'structure_template': {
                'common_chapters': [
            ...
            'statistics': {
                'avg_slide_count': 35,
                'avg_chapters': 9,
                'avg_segments_per_chapter': 3
            },
            'content_rules': {
                'bullet_threshold': 3,  # Convert to bullets if >3 points
                'combine_threshold': 100,  # Combine slides if <100 words each
                'split_threshold': 500  # Split content if >500 words
            }
        }
        ...
        print("✅ Created learned_patterns.json with sensible defaults")
    
    # Display what was learned/created
    print("\n📊 Pattern Summary:")
    print(f"  - Slide types to omit: {patterns['transformation_rules']['omit_slide_types']}")
    print(f"  - Slide types to combine: {patterns['transformation_rules']['combine_slide_types']}")
    print(f"  - Standard chapter sequence: {len(patterns['structure_template']['standard_sequence'])} chapters")
    print(f"  - Avg slides per segment: {patterns['transformation_rules']['avg_slides_per_segment']}")
    
    print("\n✅ Ready to generate storyboards using these patterns!")
    print("\nNext step: Run the pattern-based storyboard generator")
